...
# Compute summary statistics for each month separately
...
# Prepare a list to store monthly data summaries
monthly_weather_summaries <- list()

# Loop through each month and calculate weather statistics
for (month_name in names(months)) {
  month_index <- months[[month_name]]
  
  # Filter data for the specific month and calculate means
  monthly_summary <- merged_data %>%
    filter(month(Date) == month_index) %>%
    group_by(location, genotype, id, block) %>%
    summarize(
      mean_DL = mean(DL, na.rm = TRUE),
      mean_PP = mean(PP, na.rm = TRUE),
  ...
  # Store the summary in the list with an appropriate key
  monthly_weather_summaries[[month_name]] <- monthly_summary
}

# Combine all monthly summaries into one dataframe
weather_summary_by_month <- bind_rows(monthly_weather_summaries, .id = "Month")

# Display structure and a summary of the combined monthly data
str(weather_summary_by_month)
summary(weather_summary_by_month)



# Pivot data to wide format to have months as separate columns for each variable
weather_spring_month <- weather_summary_by_month %>%
  pivot_wider(
    names_from = Month,
    values_from = c(mean_DL, mean_PP, mean_Rad, mean_SWE, mean_Tmax,
                    mean_Tmin, mean_VPD, mean_Tmean, mean_ET0_HS,
                    mean_DT, mean_TT, total_GDD),
    names_sep = "_"
  )

# Check the structure and a summary of the reshaped data
str(weather_spring_month)
summary(weather_spring_month)
write.csv(weather_spring_month, row.names = FALSE, na = '', file = paste0('weather_summary_by_month.csv'))
...
# Merge the data based on common columns
merged_data_month <- left_join(agr_data, month_data, by = c("location", "genotype", "id", "block"))

# Check the structure of the merged data to ensure the join was successful
str(merged_data_month)
# Check for missing values in key columns
